...
2. **Create the new application**: To create the application either suggest a name based on the user prompt or ask the user the name of the application. Create the application with the following command:
   ...
   **IMPORTANT - Davivienda Testing Standards**: Do NOT use `--skip-tests` unless explicitly requested. This project follows Davivienda's unit testing standards which require Karma + Jasmine tests for all components and services. When generating components/services, always include the `.spec.ts` files. Follow TDD principles: write tests before or alongside implementation. For detailed testing guidelines, refer to the `angular-developer` skill's Davivienda testing standards.
