...
### Davivienda Unit Testing Standards (Priority)

This project follows Davivienda's internal unit testing standards, which use **Karma + Jasmine** as the testing framework. When writing or modifying tests, you MUST follow these guidelines:

1. **TDD approach**: Write tests before implementing functionality when possible
2. **Independent tests**: Each test must be autonomous - no shared mutable state between tests
3. **Descriptive naming**: Use clear, descriptive names that explain what is being tested
4. **`beforeEach` for setup**: Always configure `TestBed` in `beforeEach` blocks
5. **Positive and negative cases**: Cover both success and failure scenarios
6. **Spies and mocks**: Use `jasmine.createSpyObj()` and spies to isolate dependencies
7. **Run frequently**: Execute `ng test` regularly during development
8. **Refactor tests**: Maintain test code quality like production code
9. **Fast execution**: Avoid real HTTP calls, use `HttpClientTestingModule` and mocks
10. **Document complex tests**: Add comments explaining business logic behind test cases

For complete patterns, examples (services, components, HTTP), and detailed guidance, read [davivienda-testing-standards.md](references/davivienda-testing-standards.md).

### General Angular Testing References

For additional Angular testing patterns beyond the Davivienda standards:

- **Fundamentals**: Async patterns and `TestBed` configuration. Read [testing-fundamentals.md](references/testing-fundamentals.md)
